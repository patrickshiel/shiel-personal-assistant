/**
 * Minimal Todoist client. Default base is api/v1 (avoids 410 from rest/v1 and rest/v2 in many regions).
 * Override with TODOIST_API_BASE in .env if needed (e.g. https://api.todoist.com/rest/v2 for REST v2).
 */
...
/** Normalise API response to a task array (api/v1 may return array or object with results/items/tasks). */
function toTaskArray(raw: unknown): unknown[] {
...
export async function getTasks(
  ...
  const raw = await request<unknown>("GET", "tasks", token, {
    params: Object.keys(q).length ? q : undefined,
  });
  return toTaskArray(raw);
}
