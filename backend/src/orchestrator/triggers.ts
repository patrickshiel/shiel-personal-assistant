...
4. If no priority tasks or no free slots, say so.`,
  ...
  resync_day: {
    id: "resync_day",
    // no schedule - run on demand: npm run orchestrate -- --once resync_day
    systemPrompt: `${BASE_CAPABILITIES}

You are running "resync day": re-read the calendar for the given day and keep Todoist prep tasks and Obsidian prep notes in sync with the current calendar state.

Do the following in order:

1. **Fetch calendar for the day**: Call calendar_list_events_both with timeMin and timeMax covering the full day (00:00:00 to 23:59:59 in UTC for the date the user specified, or today). The list includes all events (including unaccepted invitations). You will use this as the source of truth for what meetings exist.

2. **Fetch existing Todoist tasks for the day**: Call todoist_list_tasks for both personal and work with a filter for the date (e.g. "today" or the specific date) so you can see which prep tasks already exist. Match tasks to events by content/title (e.g. "Prep for [Meeting name]").

3. **For each calendar event (meeting) on that day**:
   - **If a matching prep task exists**: Update it if anything changed—use todoist_update_task to refresh the task's content (title with due time), description (meeting details and checklist), or due_string if the meeting time changed. Ensure the task still has the due date/time at the start of the title and that the description reflects the latest meeting info.
   - **If a matching prep task does not exist**: Create one—use todoist_add_task with one task per meeting: title "YYYY-MM-DD HH:MM - Prep for [Meeting title]" (due time = start of prep block before the meeting), description with meeting name, time, and "Prep checklist:" (default: Review agenda and materials, Prepare notes; or infer from event).
   - **Obsidian**: For each event, ensure there is a prep note. If the note exists (obsidian_read_note for the expected path), use obsidian_append_to_note to add "## Update YYYY-MM-DD HH:MM" with any changed meeting context or checklist. If the note does not exist, use obsidian_write_note with path "Meetings/YYYY-MM-DD HH:MM - Prep - [Meeting name].md" (work) or "Personal/..." (personal), content: Meeting context section, then Prep checklist. Match note path to the meeting's start datetime and name.

4. **Close Todoist tasks that no longer have a matching event**: For each prep task (title containing "Prep for") on that day that does not match any event in the calendar list (e.g. the meeting was cancelled or removed), use todoist_close_task to close that task so the list stays accurate. Do not close tasks that match a current calendar event.

5. **Obsidian notes for removed events**: If you closed a Todoist task because the event was gone, you may append to the corresponding Obsidian note (if you can identify it) a line like "Meeting cancelled or removed from calendar (synced YYYY-MM-DD)." Otherwise leave notes as is.

Use the same conventions as weekly prep: one Todoist task per meeting, checklist in description, due time = enough before meeting; one Obsidian note per meeting with Meeting context and Prep checklist. Work events use context "work", personal use context "personal".`,
    defaultInput: "Resync today. Use calendar_list_events_both for today (timeMin today 00:00:00Z, timeMax today 23:59:59Z). Then for each event: update or create Todoist prep task and Obsidian prep note. Close any Todoist prep tasks for meetings that are no longer on the calendar. Update any Obsidian notes with new details.",
  },
};
...
export function getAllTriggerIds(): TriggerId[] {
  return Object.keys(TRIGGERS) as TriggerId[];
}
