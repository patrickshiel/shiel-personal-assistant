...
import { createJob, newId } from "../api/job-store.js";
...
async function proposeTriggerToJob(
  triggerId: TriggerId,
  inputOverride?: string
): Promise<string | undefined> {
  const trigger = getTrigger(triggerId);
  ...
  log(triggerId, "Starting (propose-mode)");
  ...
  try {
    ...
    const outputText = (result?.output ?? result?.output_text ?? "(no output)").toString();
    const jobId = newId(`job_${triggerId}`);
    await createJob({
      id: jobId,
      type: "trigger",
      triggerId,
    ...
    log(triggerId, `Proposed ${collector.proposals.length} writes. Job: ${jobId}`);
    if (outputText.length > 0) {
      console.log(outputText.slice(0, 500) + (outputText.length > 500 ? "…" : ""));
    }
    await recordTriggerRun(triggerId, outputText);
    return jobId;
  } catch (err) {
    log(triggerId, `Error (propose-mode): ${err instanceof Error ? err.message : String(err)}`);
    console.error(err);
    await recordTriggerRun(triggerId, undefined);
    return undefined;
  }
