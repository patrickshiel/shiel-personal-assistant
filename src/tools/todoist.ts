...
export async function listProjects(input: ListProjectsInput): Promise<string> {
  ...
  if (!token) return JSON.stringify({ error: "Todoist not configured. Set TODOIST_API_TOKEN_PERSONAL and/or TODOIST_API_TOKEN_WORK." });
  try {
...
export async function listTasks(input: ListTasksInput): Promise<string> {
  ...
  try {
    ...
    return JSON.stringify({ context: getContext(input) ?? "personal", tasks });
  } catch (e) {
    return JSON.stringify({ error: formatTodoistError(e) });
  }
}
